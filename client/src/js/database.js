...
export const putDb = async (content) => {
   ...
   const saveContent = await openDB(DB_NAME, 1);

   // TODO: Now create a variable for the transaction; again, this will be referenced below.
   const transReadWrite = saveContent.transaction(DB_NAME, 'readwrite');

   // TODO: Now create a variable for the store
   const storeReadWrite = transReadWrite.objectStore(DB_NAME);

   const request = storeReadWrite.put({ id: 1, value: content });
...
export const getDb = async () => {
   ...
   // TODO: Now create a variable for the transaction; again, this will be referenced below.
   const transReadOnly = saveContent.transaction(DB_NAME, 'readonly');

   // TODO: Now create a variable for the store
   const storeReadOnly = transReadOnly.objectStore(DB_NAME);

   // Leave the rest as-is
   const request = storeReadOnly.get(1);  // Updated from store.get
   const result = await request;
   result
